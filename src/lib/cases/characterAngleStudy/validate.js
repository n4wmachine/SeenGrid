/**
 * character_angle_study — State-Validator
 *
 * JS-Hook des Case-Bundles (Spec §8). Validiert einen character_angle_study
 * State gegen die deklarativen Constraints aus `case.json` + `schema.json`.
 * Wirft mit sprechender Fehlermeldung wenn der State inkonsistent ist.
 */
...
export function validateState(state) {
  ...
  } else if (!schema.validPanelCounts.includes(state.layout.panel_count)) {
    errors.push(
      `layout.panel_count must be one of ${schema.validPanelCounts.join(", ")} ` +
  ...
  if (state.environment !== undefined) {
    ...
    if (!schema.environmentModes.includes(state.environment.mode)) {
      errors.push(
        `environment.mode must be one of ${schema.environmentModes.join(", ")} ` +
