...
- Merge auf `main` (Code/Pilots) — **nur pro Pilot und nur mit explizitem OK** von Jonas im Chat ("merge Pilot N"). Ablauf ist in Schritt 6 des PILOT-WORKFLOW (GT-FIRST) definiert: lokaler Fast-Forward-Merge, kein PR, kein MCP, Anti-Loop-Gesetz. Pilot 1 ist bereits durch (Commit `31c08c5`), Pilot 2 folgt nach GT-Validierung
- **Ausnahme von der Merge-Regel — CLAUDE.md (Doku-Sync auf main):** CLAUDE.md ist Koordinations-Doku, keine Pilot-Arbeit. Jede Änderung an CLAUDE.md wird sofort auch auf `main` gepusht (Fast-Forward), damit jeder neue Chat beim Default-Read von `main` IMMER die aktuelle Wahrheit sieht. Kein Warten auf den nächsten Pilot-Merge, kein explizites "merge"-OK nötig. Ablauf: auf Working Branch editieren + committen, danach `git checkout main && git pull && git merge --ff-only <working-branch> && git push origin main && git checkout <working-branch>`. Gilt ausschließlich für CLAUDE.md, **nicht** für Code, Skeletons, Module, Goldens, Tests oder irgendeine andere Repo-Datei — die bleiben strikt pro-Pilot-gemerged nach Schritt 6 des GT-FIRST Workflows. Grund für diese Ausnahme: frühere Chats haben immer wieder den Default-Branch (main) gelesen und dadurch veraltete STATUS-Blöcke gesehen — dieses strukturelle Problem wird damit für Doku endgültig ausgeschaltet
